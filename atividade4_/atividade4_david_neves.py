...
@atv4.route("/alunos", methods=["GET"])
def get_alunos():
    return jsonify(lista_dados_dos_alunos)


# GET - retorna as siglas dos cursos
@atv4.route("/siglas", methods=["GET"])
def get_siglas():
    siglas = coleta_siglas(lista_dados_dos_alunos)
    return jsonify({"siglas": siglas, "total": len(siglas)})


# GET - retorna aprovados e reprovados
@atv4.route("/validacao", methods=["GET"])
def get_validacao():
    aprovados, reprovados, cont_a, cont_r = validacao_de_alunos(lista_dados_dos_alunos)
    return jsonify({
        "aprovados": aprovados,
        "total_aprovados": cont_a,
        "reprovados": reprovados,
        "total_reprovados": cont_r
    })


# POST - adiciona um novo aluno
@atv4.route("/alunos", methods=["POST"])
def add_aluno():
    novo = request.get_json()
    lista_dados_dos_alunos.append(novo)
